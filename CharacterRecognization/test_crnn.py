...
class Test_CRNN(object):
    def __init__(self, batch_size=None):
        net_params = configparser.ConfigParser()
        train_params = configparser.ConfigParser()
        net_params.read('net.cfg')
        self._model_save_path = str(net_params.get('net', 'model_load_path'))
        self.input_img_height = int(net_params.get('net', 'input_height'))
        self.input_img_width = int(net_params.get('net', 'input_width'))
        if batch_size is None:
            self.test_batch_size = int(net_params.get('net', 'test_batch_size'))
        else:
...
if __name__ == "__main__":
    ...
    test_img_list = ['00064.jpg']
    a = Test_CRNN()
    b = a.test_img(test_img_list)
    c = "".join(b)
    print(b)
    a.save_to_file('result.txt', c)
    #print(a.test_img(test_img_list))
    ...
    #     test_list.append('%05d.jpg' % (i+945))
    #     name = [('%05d' % (i+945))]
